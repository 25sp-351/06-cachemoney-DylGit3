...
// // (type of a function that) takes a provider func, returns a CACHED
// provider
// // func
// typedef ProviderFunction (*SetProvider_fptr)(ProviderFunction);

// // (type of a function that) returns NULL or a pointer to a list of
// CacheStat(s)
// // terminated by a type=END_OF_STATS stat. Caller must free the returned
// pointer typedef CacheStat *(*Stats_fptr)(void);

// /*
// ** This is the interface that a program uses to access/use a cache.
// ** It is returned by load_cache_module() and then main() calls these
// functions.
// **
